...
$(function () {
  ...
  if ($('header').length > 0) {
    var stickyheaderTop = Math.floor($('header').offset().top),
      menuH = Math.floor($('.header').outerHeight());

    function stickyheader() {
      let ww = $(window).outerWidth();
      if (ww >= wwSmall && $(this).scrollTop() > stickyheaderTop) {
    ...
    _window.on('resize', function (event) {
      clearTimeout(resizeNavTimer);
      resizeNavTimer = setTimeout(function () {
        stickyheaderTop = Math.floor($('header').offset().top);
        $('.main').removeAttr('style');
        stickyheader();
      }, 200);
    });
  ...
  window.onload = function (e) {
    ...
    //alert('cookie='+cookie);
    if (cookie == 'small') {
      //$('.font_size_block').find('.small').click();
      ...
      if (cookie == 'large') {
        //$('.font_size_block').find('.large').click();
        ...
        //這裡是預設宣告
        //$('.font_size_block').find('.medium').click();
  ...
  $('table').each(function (index, el) {
    //判斷沒有table_list
    if ($(this).parents('.table_list').length == 0 && $(this).parents('.fix_th_table').length == 0 && ($(this).parent('form').length == 0) == 0 && $(this).parents('.fix_th_table').length == 0 && $(this).parents('.detailed_information ').length == 0) {
      $(this).scroltable();
    }
  });
